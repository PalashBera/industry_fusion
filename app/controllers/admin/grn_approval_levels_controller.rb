class Admin::GrnApprovalLevelsController < Admin::HomeController
  def index
    @search = ApprovalLevel.grn.ransack(params[:q])
  ...
  def destroy
    if approval_level.destroy
      redirect_to admin_grn_approval_levels_path, flash: { success: t("flash_messages.deleted", name: "Grn Approval Level") }
    else
      redirect_to admin_grn_approval_levels_path, flash: { danger: "You can not delete this approval level because it has level users." }
    end
  end
