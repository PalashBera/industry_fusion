...
shared_examples_for "nameable" do
  ...
  describe "#validations" do
    let!(:named_resource) { create(described_class.to_s.underscore.to_sym) }
  ...
  describe "#scopes" do
    context "order_by_name" do
      ...
      it "should return records order by name" do
        expect(User.current_user.organization.public_send(described_class.to_s.pluralize.underscore).order_by_name).to eq([record_1, record_2])
      end
    end
  end
end
